...
export default function QRConnect({ active = true }) {
  const [qrData, setQrData] = useState(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(null)

  useEffect(() => {
    if (active) fetchQRData()
  }, [active])
  ...
  const qrString = qrData?.local ? JSON.stringify(qrData) : null

  return (
    <div className="qr-connect">
      <DesktopPairing active={active} />
      <PairedDevices />

      <div className="qr-connect-divider"><span>or</span></div>
      ...
      {loading ? (
        ...
        <div className="qr-notice">
          <p>Enable local network access in Network settings to generate this legacy mobile-connect QR.</p>
        </div>
      ) : (
        <>

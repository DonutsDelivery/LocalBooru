...
const LEGACY_GENERATED_DEVICE_NAMES = new Set(['MacIntel Desktop', 'Linux x86_64 Desktop', 'Win32 Desktop'])

export function defaultDesktopName(userAgent = navigator.userAgent, platform = navigator.platform) {
  const identity = `${userAgent || ''} ${platform || ''}`
  const family = /Linux/i.test(identity) ? 'Linux' : /Windows|Win32/i.test(identity) ? 'Windows' : /Mac/i.test(identity) ? 'Mac' : 'LocalBooru'
  return `${family} Desktop`
}

export function initialDesktopName(storedName, detectedName) {
  return !storedName || LEGACY_GENERATED_DEVICE_NAMES.has(storedName) ? detectedName : storedName
}

const DEFAULT_DEVICE_NAME = defaultDesktopName()

export default function DesktopPairing({ active = true }) {
  const [deviceName, setDeviceName] = useState(() => initialDesktopName(localStorage.getItem('localbooru_device_name'), DEFAULT_DEVICE_NAME))

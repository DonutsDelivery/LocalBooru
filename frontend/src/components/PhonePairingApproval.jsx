...
import { authorizeServerForDesktop, confirmDeviceAuthorization, formatPairingFingerprint, isEligiblePairingServerUrl } from '../devicePairing'
...
export default function PhonePairingApproval({ request, servers, onClose, onComplete }) {
  ...
  useEffect(() => {
    setSelected(previous => {
      const eligibleIds = new Set(compatibleServers.map(server => server.id))
      const retained = new Set([...previous].filter(id => eligibleIds.has(id)))
      if (compatibleServers.length === 1) retained.add(compatibleServers[0].id)
      return retained
    })
  }, [compatibleServers])
        ...
        {!finished && (
          <>
            ...
            <div className="pairing-server-choices">
              ...
              {servers.some(server => !server.token || !isEligiblePairingServerUrl(server.url)) && <p className="pairing-warning">Servers without a device token, or public servers using unencrypted HTTP, are not eligible.</p>}
            </div>

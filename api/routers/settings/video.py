...
async def play_video_transcode(request: TranscodePlayRequest):
    ...
    the user wants to change quality/bitrate. Also handles VFR→CFR conversion.
    """
    ...
    try:
        # Detect source resolution and VFR status
        source_resolution = None
        is_vfr = False
        try:
            result = subprocess.run([
            ...
            # Detect VFR
            vfr_info = detect_vfr(request.file_path)
            is_vfr = vfr_info.get("is_vfr", False)
            if is_vfr:
                print(f"[Transcode] VFR video detected: r_fps={vfr_info.get('r_fps')}, avg_fps={vfr_info.get('avg_fps')}")
        except Exception as e:
            print(f"[Transcode] Could not detect source resolution: {e}")

        quality_settings = parse_quality_preset(request.quality_preset, source_resolution)

        # Create transcode stream (with VFR→CFR conversion if needed)
        stream = TranscodeStream(
            video_path=request.file_path,
            target_bitrate=quality_settings["bitrate"],
            target_resolution=quality_settings["resolution"],
            start_position=request.start_position,
            force_cfr=is_vfr,  # Convert VFR to CFR
        )
...
def detect_vfr(file_path: str) -> dict:
    """
    Detect if a video is Variable Frame Rate (VFR).

    VFR videos have inconsistent frame timing which can cause stuttering
    in browser playback. We detect this by comparing r_frame_rate
    (container time base) vs avg_frame_rate (actual average).

    Returns:
        {
            "is_vfr": bool,
            "r_frame_rate": str,  # Container frame rate (e.g., "300/1" for VFR)
            "avg_frame_rate": str,  # Average actual frame rate (e.g., "25000/1001")
            "r_fps": float,
            "avg_fps": float,
        }
    """
    try:
        ...
        if result.returncode != 0:
            return {"is_vfr": False, "error": "Failed to probe video"}

        data = json.loads(result.stdout)
        streams = data.get("streams", [])
        if not streams:
            return {"is_vfr": False, "error": "No video stream found"}
        ...
        # VFR detection heuristics:
        # 1. r_frame_rate >> avg_frame_rate (e.g., 300 vs 25)
        # 2. r_frame_rate is a common VFR time base (120, 240, 300, 600, 1000, etc.)
        # 3. The ratio between r_fps and avg_fps is significantly different

        is_vfr = False

        # Common VFR time bases (container frame rates)
        vfr_time_bases = {60, 90, 120, 180, 240, 300, 360, 480, 600, 1000}

        if r_fps > 0 and avg_fps > 0:
            ratio = r_fps / avg_fps
            # If r_fps is much higher than avg_fps (ratio > 2), likely VFR
            # Also check if r_fps is a known VFR time base
            is_vfr = (ratio > 2.0) or (int(r_fps) in vfr_time_bases and ratio > 1.5)
        ...
        return {"is_vfr": False, "error": str(e)}


@router.post("/video-info")
async def get_video_info(request: VideoInfoRequest):
    """
    Get video information including VFR detection.

    This is used by the frontend to determine if a video needs
